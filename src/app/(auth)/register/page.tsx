...
export default function RegisterPage() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    toast.success('חשבון נוצר! בדוק את האימייל שלך לאישור')
    router.push('/login')
  }
  ...
  return (
    <div className="bg-white dark:bg-[#1e293b] rounded-2xl p-8 shadow-[0_12px_40px_rgba(0,36,82,0.10)] dark:shadow-[0_12px_40px_rgba(0,0,0,0.3)] border border-[#c4c6d0]/20 dark:border-[#334155]">
      {/* Header */}
      <div className="mb-8">
        <h2 className="text-2xl font-headline font-bold text-[#002452] dark:text-white">הרשמה</h2>
        <p className="text-[#44474f] dark:text-[#94a3b8] text-sm mt-1">צור חשבון חדש</p>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-5">
        ...
        <div className="space-y-1.5">
          ...
          <div className="relative">
            <span className="material-symbols-outlined absolute right-3 top-1/2 -translate-y-1/2 text-[#747780] dark:text-[#94a3b8] text-xl pointer-events-none">person</span>
            <input
              type="text"
              value={fullName}
        ...
        >
          {loading ? (
            ...
            <>
              <span className="material-symbols-outlined filled text-xl">how_to_reg</span>
              הרשמה
            </>
          )}
        </button>
      ...
      <p className="text-center text-sm text-[#44474f] dark:text-[#94a3b8] mt-8">
        יש לך חשבון?{' '}
        <Link href="/login" className="text-[#0051d5] dark:text-sky-400 hover:underline font-semibold">
          כניסה כאן
        </Link>
      </p>
    </div>
